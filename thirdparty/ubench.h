...
#define UBENCH_INITIALIZER(f)                                                  \
  ...
  UBENCH_INITIALIZER_BEGIN_DISABLE_WARNINGS __pragma(                          \
    ...
    UBENCH_C_FUNC __declspec(allocate(".CRT$XCU")) void(__cdecl * f##_)(       \
      void) = f;                                                               \
...
static UBENCH_INLINE ubench_int64_t ubench_ns(void) {
  ...
  return UBENCH_CAST(
    ubench_int64_t, (counter.QuadPart * 1000000000) / frequency.QuadPart);
...
#define UBENCH_EX(SET, NAME)                                                   \
  ...
  UBENCH_INITIALIZER(ubench_register_##SET##_##NAME) {                         \
    ...
    ubench_state.benchmarks = UBENCH_PTR_CAST(                                 \
      struct ubench_benchmark_state_s *,                                       \
      realloc(                                                                 \
        UBENCH_PTR_CAST(void *, ubench_state.benchmarks),                      \
...
#define UBENCH_EX_F(FIXTURE, NAME)                                             \
  ...
  static void ubench_run_ex_##FIXTURE##_##NAME(                                \
    struct FIXTURE *, struct ubench_run_state_s *);                            \
...
static UBENCH_INLINE int
ubench_should_filter(const char *filter, const char *benchmark);
...
static UBENCH_INLINE int
ubench_strncmp(const char *a, const char *b, size_t n) {
...
static UBENCH_INLINE FILE *
ubench_fopen(const char *filename, const char *mode) {
#ifdef _MSC_VER
  FILE *file;
...
int ubench_main(int argc, const char *const argv[]) {
  ...
  for (index = 1; index < UBENCH_CAST(size_t, argc); index++) {
    ...
    if (0 == ubench_strncmp(argv[index], help_str, strlen(help_str))) {
      ...
      0 == ubench_strncmp(argv[index], filter_str, strlen(filter_str))) {
      /* user wants to filter what benchmarks run! */
      filter = argv[index] + strlen(filter_str);
    } else if (
      0 == ubench_strncmp(argv[index], output_str, strlen(output_str))) {
      ubench_state.output =
      ...
      0 ==
      ubench_strncmp(argv[index], confidence_str, strlen(confidence_str))) {
  ...
  printf(
    "%s[==========]%s Running %" UBENCH_PRIu64 " benchmarks.\n", colours[GREEN],
    colours[RESET], UBENCH_CAST(ubench_uint64_t, ran_benchmarks));

  if (ubench_state.output) {
    fprintf(
      ubench_state.output, "name, mean (ns), stddev (%%), confidence (%%)\n");
  }
...
#undef UBENCH_MIN_ITERATIONS
    ...
    if (result) {
      printf(
        "confidence interval %f%% exceeds maximum permitted %f%%\n",
        best_confidence, ubench_state.confidence);
    }

    if (ubench_state.output) {
      fprintf(
        ubench_state.output, "%s, %" UBENCH_PRId64 ", %f, %f,\n",
...

    {
      ...
      if (0 != result) {
        ...
        failed_benchmarks = UBENCH_PTR_CAST(
          size_t *, realloc(
                      UBENCH_PTR_CAST(void *, failed_benchmarks),
      ...
      printf(
        "%s%s%s %s (mean ", colour, status, colours[RESET],
        ubench_state.benchmarks[index].name);
      ...
      printf(
        "%" UBENCH_PRId64 ".%03" UBENCH_PRId64
  ...
  printf(
    "%s[==========]%s %" UBENCH_PRIu64 " benchmarks ran.\n", colours[GREEN],
    colours[RESET], ran_benchmarks);
  printf(
    "%s[  PASSED  ]%s %" UBENCH_PRIu64 " benchmarks.\n", colours[GREEN],
    colours[RESET], ran_benchmarks - failed);

  if (0 != failed) {
    printf(
      "%s[  FAILED  ]%s %" UBENCH_PRIu64 " benchmarks, listed below:\n",
