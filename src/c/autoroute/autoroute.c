  ...
  while (circ_lliter_next(&netit)) {
    ...
    while (circ_lliter_next(&subnetit)) {
      ...
      while (circ_lliter_next(&endpointit)) {
        ...
        log_debug("Endpoint %x: %d %d", endpointID, endpoint.position.x,
                  endpoint.position.y);
        ...
        log_debug("Anchor %x: %d %d", endpointID, anchor.position.x,
                  anchor.position.y);
...
static void autoroute_prepare_routing(AutoRoute *ar, RoutingConfig config) {
  ...
  RT_Result res = RT_graph_build(
      ...
      (RT_Slice_BoundingBox){ar->boxes, arrlen(ar->boxes)},
...
bool autoroute_dump_routing_data(AutoRoute *ar, RoutingConfig config,
                                 const char *filename) {
...
void draw_stroked_line(DrawContext *draw, HMM_Vec2 start, HMM_Vec2 end,
                       float line_thickness, HMM_Vec4 color);
void draw_stroked_line(DrawContext *draw, HMM_Vec2 start, HMM_Vec2 end,
                       float line_thickness, HMM_Vec4 color);
void draw_filled_circle(DrawContext *draw, HMM_Vec2 position, HMM_Vec2 size,
                        HMM_Vec4 color);
void draw_text(DrawContext *draw, Box rect, const char *text, int len,
               float fontSize, FontHandle font, HMM_Vec4 fgColor,
               HMM_Vec4 bgColor);
Box draw_text_bounds(DrawContext *ctx, HMM_Vec2 pos, const char *text, int len,
                     HorizAlign horz, VertAlign vert, float fontSize,
                     FontHandle font);
void draw_stroked_circle(DrawContext *draw, HMM_Vec2 position, HMM_Vec2 size,
                         float line_thickness, HMM_Vec4 color);
void draw_stroked_rect(DrawContext *draw, HMM_Vec2 position, HMM_Vec2 size,
                       float radius, float line_thickness, HMM_Vec4 color);
...
void autoroute_draw_debug_lines(AutoRoute *ar, DrawContext *ctx) {
  ...
  for (size_t i = 0; i < nodes.len; i++) {
    ...
    draw_filled_circle(ctx, HMM_V2((float)p1.x - 1.5, (float)p1.y - 1.5),
                       HMM_V2(3, 3), HMM_V4(0.5f, 1.0f, 1.0f, 0.5f));
    for (size_t j = 0; j < 2; j++) {
      if (neighbors[j] < nodes.len) {
        RT_Point p2 = nodes.ptr[neighbors[j]].position;
        draw_stroked_line(ctx, HMM_V2(p1.x, p1.y), HMM_V2(p2.x, p2.y), 0.5,
  ...
  for (size_t i = 0; i < arrlen(ar->boxes); i++) {
    ...
    HMM_Vec2 tl = HMM_V2(box->center.x - box->half_width,
                         box->center.y - box->half_height);
    // HMM_Vec2 br =
    //   HMM_V2(box->center.x + box->half_width, box->center.y +
    //   box->half_height);
    draw_stroked_rect(ctx, tl,
                      HMM_V2(box->half_width * 2.0f, box->half_height * 2.0f),
                      0, 1, HMM_V4(0.7f, 0.5f, 0.5f, 0.5f));
    // draw_stroked_line(
...
static void autoroute_replay_clear_state(AutoRoute *ar) {
  ...
  while (hmlen(rec->poppedScores) > 0) {
    hmdel(rec->poppedScores,
          rec->poppedScores[hmlen(rec->poppedScores) - 1].key);
  }
  while (hmlen(rec->predecessors) > 0) {
    hmdel(rec->predecessors,
          rec->predecessors[hmlen(rec->predecessors) - 1].key);
  }
}
...
static RT_Point autoroute_replay_closest_point_on_line(RT_Point A, RT_Point B,
                                                       RT_Point P) {
  RT_Point closest;
...
static bool autoroute_replay_play(AutoRoute *ar) {
  ...
  case REC_EVENT_PATH_FINDING_SET_G_SCORE:
    hmput(rec->gScores, event->path_finding_set_g_score.node,
          event->path_finding_set_g_score.g_score);
    break;

  case REC_EVENT_PATH_FINDING_PUSH_OPEN_QUEUE:
    hmput(rec->fScores, event->path_finding_push_open_queue.node,
          event->path_finding_push_open_queue.f_score);
    break;

  case REC_EVENT_PATH_FINDING_SET_PREDECESSOR:
    hmput(rec->predecessors, event->path_finding_set_predecessor.node,
  ...
  case REC_EVENT_PATH_FINDING_INSERT_PATH_NODE:
    if (event->path_finding_insert_path_node.index >= 0 &&
        event->path_finding_insert_path_node.index < arrlen(rec->path)) {
      arrins(rec->path, event->path_finding_insert_path_node.index,
             event->path_finding_insert_path_node.node);
    } else if (event->path_finding_insert_path_node.index ==
               arrlen(rec->path)) {
      arrput(rec->path, event->path_finding_insert_path_node.node);
    } else {
      log_error("Insert: Invalid path node index %zu / %zu",
                event->path_finding_insert_path_node.index, arrlen(rec->path));
    }
...
bool autoroute_replay_forward_skip_path(AutoRoute *ar) {
  ...
  for (;;) {
    ...
    if (ar->recording.events[ar->recording.currentEvent - 1].type ==
        REC_EVENT_BEGIN_PATH_FINDING) {
      return true;
    }
  }
...
bool autoroute_replay_forward_skip_root(AutoRoute *ar) {
  ...
  for (;;) {
    ...
    if (ar->recording.events[ar->recording.currentEvent].type ==
        REC_EVENT_ROUTING_BEGIN_ROOT_WIRE) {
      return true;
    }
  }
...
bool autoroute_replay_backward_skip_path(AutoRoute *ar) {
  ...
  while (currentEvent >= 0) {
    currentEvent--;
    if (ar->recording.events[currentEvent].type ==
        REC_EVENT_BEGIN_PATH_FINDING) {
      break;
    }
  }
...
void autoroute_replay_draw(AutoRoute *ar, DrawContext *ctx, FontHandle font) {
  ...
  if (rec->rootWireValid) {
    draw_stroked_line(ctx, pt2vec2(rec->rootWireStart),
                      pt2vec2(rec->rootWireEnd), 1.0,
                      HMM_V4(0.5f, 0.0f, 1.0f, 0.5f));
  }

  if (rec->branchWireValid) {
    draw_stroked_line(ctx, pt2vec2(rec->branchWireStart),
                      pt2vec2(rec->branchWireEnd), 1.0,
  ...
  for (size_t i = 0; i < arrlen(rec->routePaths); i++) {
    ...
    for (size_t j = path->start + 1; j < path->end; j++) {
      ...
      draw_stroked_line(ctx, HMM_V2(v1.x, v1.y), HMM_V2(v2.x, v2.y), 1.5,
  ...
  if (rec->inPathFinding) {
    for (size_t i = 0; i < hmlen(rec->predecessors); i++) {
      ...
      draw_stroked_line(ctx, pt2vec2(p1), pt2vec2(p2), 0.5,
                        HMM_V4(0.5f, 0.5f, 0.5f, 0.5f));
    }
    ...
    for (size_t i = 1; i < arrlen(rec->path); i++) {
      ...
      draw_stroked_line(ctx, pt2vec2(p1), pt2vec2(p2), 0.8,
                        HMM_V4(0.2f, 1.0f, 0.2f, 0.5f));
    }

    RT_Point start = rec->graph.ptr[rec->startNode].position;
    draw_stroked_circle(ctx, HMM_V2(start.x - 3.5, start.y - 3.5), HMM_V2(7, 7),
                        2.0f, HMM_V4(1.0f, 0.2f, 0.2f, 0.5f));

    for (size_t i = 0; i < arrlen(rec->endNodes); i++) {
      RT_Point end = rec->graph.ptr[rec->endNodes[i]].position;
      draw_stroked_circle(ctx, HMM_V2(end.x - 3.5, end.y - 3.5), HMM_V2(7, 7),
                          2.0f, HMM_V4(0.2f, 0.2f, 1.0f, 0.5f));
    }
    ...
    for (ptrdiff_t i = 0; i < hmlen(rec->gScores); i++) {
      ...
      draw_filled_circle(ctx, HMM_V2((float)p.x - 2.5, (float)p.y - 2.5),
                         HMM_V2(5, 5), HMM_V4(0.5f, 1.0f, 1.0f, 0.5f));
      char buf[32];
      snprintf(buf, sizeof(buf), "%d", gScore);

      Box bounds =
          draw_text_bounds(ctx, HMM_V2(p.x + 3, p.y - 3), buf, strlen(buf),
                           ALIGN_LEFT, ALIGN_BOTTOM, 4.0f, font);

      draw_text(ctx, bounds, buf, strlen(buf), 4.0f, font,
    ...
    for (ptrdiff_t i = 0; i < hmlen(rec->fScores); i++) {
      ...
      RT_Point p = rec->graph.ptr[node].position;
      draw_filled_circle(ctx, HMM_V2((float)p.x - 2.5, (float)p.y - 2.5),
                         HMM_V2(5, 5), HMM_V4(1.0f, 0.5f, 1.0f, 0.5f));
      char buf[32];
      snprintf(buf, sizeof(buf), "%d", fScore);

      Box bounds =
          draw_text_bounds(ctx, HMM_V2(p.x + 3, p.y + 3), buf, strlen(buf),
                           ALIGN_LEFT, ALIGN_TOP, 4.0f, font);
    ...
    for (ptrdiff_t i = 0; i < hmlen(rec->poppedScores); i++) {
      ...
      RT_Point p = rec->graph.ptr[node].position;
      draw_filled_circle(ctx, HMM_V2((float)p.x - 2.5, (float)p.y - 2.5),
                         HMM_V2(5, 5), HMM_V4(0.5f, 1.0f, 0.5f, 0.5f));
      char buf[32];
      snprintf(buf, sizeof(buf), "%d", fScore);
...
void autoroute_replay_event_text(AutoRoute *ar, char *buf, size_t maxlen) {
  ...
  switch (event->type) {
  case REC_EVENT_BEGIN_PATH_FINDING:
    snprintf(buf, maxlen, "Begin path finding from %d",
             event->begin_path_finding.start_index);
    break;

  case REC_EVENT_PATH_FINDING_SET_G_SCORE:
    snprintf(buf, maxlen, "Set G score for %d to %d",
             event->path_finding_set_g_score.node,
             event->path_finding_set_g_score.g_score);
    break;

  case REC_EVENT_PATH_FINDING_PUSH_OPEN_QUEUE:
    snprintf(buf, maxlen, "Push %d to open queue with F score %d",
             event->path_finding_push_open_queue.node,
             event->path_finding_push_open_queue.f_score);
    break;

  case REC_EVENT_PATH_FINDING_SET_PREDECESSOR:
    snprintf(buf, maxlen, "Set predecessor of %d to %d",
             event->path_finding_set_predecessor.node,
             event->path_finding_set_predecessor.predecessor);
    break;

  case REC_EVENT_PATH_FINDING_POP_OPEN_QUEUE:
    snprintf(buf, maxlen, "Pop %d from open queue",
  ...
  case REC_EVENT_PATH_FINDING_INSERT_PATH_NODE:
    snprintf(buf, maxlen, "Insert %d into path at index %zu",
             event->path_finding_insert_path_node.node,
             event->path_finding_insert_path_node.index);
    break;

  case REC_EVENT_PATH_FINDING_REMOVE_PATH_NODE:
    snprintf(buf, maxlen, "Remove node at index %zu from path",
  ...
  case REC_EVENT_ROUTING_BEGIN_ROOT_WIRE:
    snprintf(buf, maxlen, "Begin root wire from (%d, %d) to (%d, %d)",
  ...
  case REC_EVENT_ROUTING_BEGIN_BRANCH_WIRE:
    snprintf(buf, maxlen, "Begin branch wire from %d, %d",
             event->routing_begin_branch_wire.start.x,
             event->routing_begin_branch_wire.start.y);
    break;

  case REC_EVENT_ROUTING_PUSH_VERTEX:
    snprintf(buf, maxlen, "Push vertex %.1f, %.1f to wire",
             event->routing_push_vertex.vertex.x,
             event->routing_push_vertex.vertex.y);
    break;
...
void autoroute_dump_anchor_boxes(AutoRoute *ar) {
  ...
  for (size_t i = 0; i < arrlen(ar->anchors); i++) {
    fprintf(fp, "    Anchor {\n");
    fprintf(fp, "        position: Point { x: %d, y: %d },\n",
    ...
    if (ar->anchors[i].connect_directions == RT_DIRECTIONS_ALL) {
      ...
      fprintf(fp, "        connect_directions: Directions::%s,\n",
              ar->anchors[i].connect_directions == RT_DIRECTIONS_POS_X
                  ? "POS_X"
                  : "NEG_X");
    }
    fprintf(fp, "    },\n");
  ...
  for (size_t i = 0; i < arrlen(ar->boxes); i++) {
    ...
    fprintf(fp, "        center: Point { x: %d, y: %d },\n",
            ar->boxes[i].center.x, ar->boxes[i].center.y);
  ...
  for (size_t i = 0; i < arrlen(ar->endpoints); i++) {
    ...
    // }
    fprintf(fp, "        position: Point { x: %d, y: %d },\n",
            endpoint->position.x, endpoint->position.x);
    fprintf(fp, "    },\n");
  }
  fprintf(fp, "];\n");
...
static void rec_begin_path_finding(void *user, RT_NodeIndex start_index,
                                   struct RT_Slice_NodeIndex end_indices,
                                   bool visit_all) {
...
static void rec_path_finding_set_g_score(void *user, RT_NodeIndex index,
                                         uint32_t g_score) {
  RouteRecording *rec = user;
  RecEvent event = {
...
static void rec_path_finding_push_open_queue(void *user, RT_NodeIndex node,
                                             uint32_t f_score) {
  RouteRecording *rec = user;
  RecEvent event = {
...
static void rec_path_finding_set_predecessor(void *user, RT_NodeIndex node,
                                             RT_NodeIndex predecessor) {
  RouteRecording *rec = user;
  RecEvent event = {
      .type = REC_EVENT_PATH_FINDING_SET_PREDECESSOR,
      .path_finding_set_predecessor = {.node = node,
...
static void rec_path_finding_insert_path_node(void *user, size_t insert_index,
                                              RT_NodeIndex node) {
  RouteRecording *rec = user;
  RecEvent event = {
...
static void rec_routing_begin_root_wire(void *user, struct RT_Point start,
                                        struct RT_Point end) {
  RouteRecording *rec = user;
  RecEvent event = {
